...
# =============================================================================
# 内核 10（warp 级分块版）参数搜索脚本
#
# 与 kernel_9 的 autotuner 思路相同：枚举所有参数组合 -> 筛掉非法组合 ->
# sed 改源码常量 -> make 重编 -> 跑基准记录结果。但 kernel 10 是 warp 级
# 分块（warptiling）内核，参数更多、约束更复杂：
#
# 参数含义：
...
# （正则匹配 "const uint K10_XXX = ..." 整行并替换成新值）
